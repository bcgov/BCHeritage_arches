...
class BCTileserverProxyView(TileserverProxyView):
    ...
    def is_upstream_local(self):
        return re.search("(localhost|127.0.0.1)", self.upstream) is not None
    ...
    def get_request_headers(self):
        ...
        if hasattr(settings, 'TILESERVER_OUTBOUND_PROXY') and settings.TILESERVER_OUTBOUND_PROXY and not self.is_upstream_local():
            # print("Setting outbound proxy to %s" % settings.TILESERVER_OUTBOUND_PROXY)
            self.http = urllib3.ProxyManager(settings.TILESERVER_OUTBOUND_PROXY)
