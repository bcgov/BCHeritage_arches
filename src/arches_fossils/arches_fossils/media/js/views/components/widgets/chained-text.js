define([
    ...
    const viewModel = function(params) {
        ...
        self.controlWidgetAlias = ko.observable(self.config().controlWidgetAlias);
        self.controlWidgetValue = ko.observable(self.config().controlWidgetValue);
        ...
        // self.tileChanged = ko.observable();
        if (!!self.bc_card)
        {
            let parentNode = _.find(self.bc_card.nodes(), function(node){ return node.alias() === self.controlWidgetAlias()});
            self.parentValue(ko.unwrap(self.tile.data[parentNode.nodeid]));
        }


        if (!!params.bc_card && !!params.bc_card.controlData)
        {
            console.log("Chained-text: Got control data!");
            // params.bc_card.controlData.subscribe(function(value){
        ...
        // if (!!self.controlWidgetAlias() && !!self.widget && self.widget.card)
        // {
        //     var parentWidget = _.find(self.widget.card.widgets(), function(widget){return widget.node.alias() === self.controlWidgetAlias() })
        //     if (!!parentWidget)
        //     {
        //         self.parentValue = ko.observable(self.tile.data[parentWidget.node.nodeid]());
        //         self.tileChanged = ko.observable(ko.unwrap(self.tile.dirty));
        //     }
        // }
        ...
        self.controlWidgetValue.subscribe(newValue => {
            self.config().controlWidgetValue = newValue;
        });

        self.controlWidgetAlias.subscribe(newValue => {
            self.config().controlWidgetAlias = newValue;
        });
        ...
        self.showWidget = ko.pureComputed(function() {
            return ko.unwrap(self.parentValue) === ko.unwrap(self.controlWidgetValue);
        }, self);
