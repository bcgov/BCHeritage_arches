define([
    ...
    /**
    * A BC Fossils view model used for generic cards
    *
    * @constructor
    * @name CardViewModel
    *
    ...
    */
    ...
    var CardViewModel = function(params) {
        ...
        self.controlData = ko.observable({});
        self.controlData.subscribe(function(){
            console.log("Hi from card: "+ self.controlData());
        });
        ...
        var applyChainedComputed = function(widgets){
            widgets.forEach(function(widget) {
                if (!!widget.config.controlWidgetAlias)
                {
                    console.log(`Found Child Widget: ${widget}`)
                    var controlWidget = _.find(widgets, function (parentWidget) {
                        return ko.unwrap(parentWidget.node.alias) === ko.unwrap(widget.config.controlWidgetAlias)
                    });
                    var controlNode = _.find(self.params.card.nodes(), function (parentNode) {
                        return ko.unwrap(parentNode.alias) === ko.unwrap(widget.config.controlWidgetAlias)
                    });
                    // controlWidget.isDirty()
                    if (!!controlNode)
                    {
                        console.log(controlNode);
                        controlNode.dirty.subscribe(function() {
                            console.log("Dirty!");
                        });
                        // self.controlData.push(self.params.tiles[0].data[controlNode.nodeid]);
                        // self.controlData.subscribe(function(newValue){
                        //     widget.showWidget(newValue);
                        // });
                        // controlNode.dirty.subscribe(function(newValue) {
                        //     widget.showWidget(newValue);
                        // });
                    }
                    ...
                    // if (!!controlWidget)
                    // {
                    //     console.log(controlWidget);
                    //     widget.subscribe(controlWidget.n)
                    // }
                }
            });
        };

        applyChainedComputed(cardModel.widgets());
        ...
        _.extend(this, nodegroup, {
            ...
            bubbleDown: ko.pureComputed(function() {
