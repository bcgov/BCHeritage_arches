...
class HriaDao:
    ...
    def validate_borden_number(self, borden_number, resourceinstanceid, conn=None):
        ...
        with connection.cursor() as cursor:
            row = cursor.execute("""select bordennumber, importpk from SDE.A101 where bordennumber = :borden_number 
                                    union select bordennumber, importpk from SDE.TFM_SITE where bordennumber = :borden_number""", borden_number=borden_number).fetchone()
            logger.debug("Exists? %s" % (row is not None))
            if row is not None and str(row[1]) != str(resourceinstanceid):
                raise ValueError("Borden Number %s already exists in HRIA" % borden_number)

            row = cursor.execute("""select bordennumber from SDE.A101 where IMPORTPK = :resource_id union 
                 select bordennumber from SDE.TFM_SITE where IMPORTPK = :resource_id""",
                                 resource_id=resourceinstanceid).fetchone()
            if row is not None and row[0] != borden_number:
                raise ValueError("Heritage Site %s is already associated with Borden Number %s in HRIA" % (resourceinstanceid, row[0]))
    ...
    def reserve_borden_number(self, borden_number, is_heritage_site, resourceinstanceid):
        ...
        logger.debug("Borden number: %s, resourceinstanceid: %s, is_heritage_site: %s" % (borden_number, resourceinstanceid, is_heritage_site))
        connection = self.__get_connection()
        with connection.cursor() as cursor:
            try:
                # Do one last validation prior to trying to reserve the borden number
                self.validate_borden_number(borden_number, resourceinstanceid, connection)
                version_exists = cursor.execute("""SELECT COUNT (1) FROM sde.versions WHERE name = :bcrhp_user""", bcrhp_user=hria_application_user).fetchone()
                ...
                cursor.callproc('sde.version_util.set_current_version', [hria_application_user])
                cursor.callproc('sde.version_user_ddl.edit_version', [hria_application_user, 1])
                ...
                cursor.execute("""INSERT INTO SDE.TFM_SYS_RECONCILE_LOG
                                        (VERSION, 
                                        VERSION_PARENT, 
                                        RECONCILE_STATUS, 
                                        RECONCILE_SUBMIT, 
                ...
                raise RuntimeError("Unable to reserve borden number. Please check logs for root cause.")
