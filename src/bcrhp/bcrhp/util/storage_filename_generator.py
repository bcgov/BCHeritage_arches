...
def generate_filename(instance, filename):
    ...
    if not hasattr(generate_filename, 'borden_number_node'):
        generate_filename.borden_number_node = models.Node.objects.filter(alias="borden_number").first()
    ...
    if not hasattr(generate_filename, 'borden_number_datatype'):
        generate_filename.borden_number_datatype = datatypes.datatypes.DataTypeFactory().get_instance(generate_filename.borden_number_node.datatype)
        # print("Got borden number datatype: %s" % str(generate_filename.borden_number_datatype))

    borden_number_tile = models.TileModel.objects.filter(resourceinstance=instance.tile.resourceinstance, nodegroup=generate_filename.borden_number_node.nodegroup).first()
    # print("Got borden number tile: %s" % str(borden_number_tile))
    borden_number = None

    paths = []
    if borden_number_tile:
        borden_number = generate_filename.borden_number_datatype.get_display_value(borden_number_tile, generate_filename.borden_number_node)
        paths = borden_number.split("-") if borden_number and "-" in borden_number else [str(instance.tile.resourceinstance.resourceinstanceid)]
    else:
