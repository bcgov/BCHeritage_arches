...
def add_permissions(apps, schema_editor, with_create_permissions=True):
    ...
    for resource in resources:
        print("Resource: %s, %s" % (resource.get_descriptor("name", {}), resource.graph.slug))
        print("User: %s, group: %s " % (get_perms(user, resource), get_perms(guest_group, resource)))
    ...
    call_command('es', 'index_resources_by_type', resource_types=[graph.graphid for graph in graphs])
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("bcrhp", "694_apply_admin_only_function"),
    ]
